...
version cannot be republished. 4.0.9 aligns the package with the repository. No changes
to the astronomical calculation engine (`core/CaelisEngine.js` is unchanged).
...
- `docs/CAELIS_ENGINE_MATEMATICA_v4_0.md`: section 2.6 names the Saemundsson (1986) formula
  (R in arcminutes) instead of Bennett (1982) and cites both papers.
- `docs/SCIENTIFIC_VALIDATION.md`: integrity hash updated.
...
`getSnapshotAt()` is not perfectly deterministic at the sub-arcsecond level: its output
depends on the wall clock at the moment it is called (tracked in detail in issue #4).
This is far below the engine's declared precision and does not affect any published
astronomical value, but it does not meet the strict determinism Caelis Engine is
designed around. A fix is being designed carefully so it does not compromise that
guarantee, and will ship in a future release once fully validated.

### Validation
- Core suite: 31/31 · Extended benchmarks: 67/67
- Polar safety: 256/256 · ASC/MC: 104/104 · CJS interop: 18/18
- Total: 476/476 assertions
